...
export const beef = document.getElementById("Beef");
export const Chicken = document.getElementById("Chicken");
export const Dessert = document.getElementById("Dessert");
export const Lamb = document.getElementById("Lamb");
export const Starter = document.getElementById("Starter");
export const Pasta = document.getElementById("Pasta");
export const Pork = document.getElementById("Pork");
export const Seafood = document.getElementById("Seafood");
export const Vegan = document.getElementById("Vegan");
export const Breakfast = document.getElementById("Breakfast");
export const Goat = document.getElementById("Goat");
...
export async function filtercategory(category) {
  try {
    ...
    categoryfilter = response.results;
...
function displayfiltercategory() {
  ...
  for (let i = 0; i < categoryfilter.length; i++) {
    ...
    onclick=" layerappear3(${i}) "
              class="recipe-card bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all cursor-pointer group"
              data-meal-id="${categoryfilter[i].id}"
            >
              <div class="relative h-48 overflow-hidden">
                <img
                  class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
                  src="${categoryfilter[i].thumbnail}"
                  loading="lazy"
                />
                <div class="absolute bottom-3 left-3 flex gap-2">
                  ...
                  <span
                    class=" ${categoryfilter[i].area == null ? "loading" : ""}  px-2 py-1 bg-emerald-500 text-xs font-semibold rounded-full text-white"
                  >
                     ${categoryfilter[i].area}
                  </span>
                </div>
              </div>
              <div class="p-4">
                ...
                >
                   ${categoryfilter[i].name}
                </h3>
                <p class="text-xs text-gray-600 mb-3 line-clamp-2">
                   ${categoryfilter[i].instructions}
                </p>
                <div class="flex items-center justify-between text-xs">
                  ...
                  <span class="font-semibold text-gray-500">
                    <i class="fa-solid fa-globe text-blue-500 mr-1"></i>
                     ${categoryfilter[i].area == null ? "international" : categoryfilter[i].area} 
                  </span>
                </div>
              </div>
            </div>
  ...
  count.innerHTML = ` Showing ${categoryfilter.length} recipes`;
  recipes.classList.add("grid", "grid-cols-4", "gap-5");
  recipes.innerHTML = cart;
}

window.layerappear3 = function (index) {
  let cart = "";
  cart = `
          ...
          <div class="bg-white rounded-2xl shadow-lg overflow-hidden mb-8">
            <div class="relative h-80 md:h-96">
              <img
                src="${categoryfilter[index].thumbnail}"
              ...
              <div class="absolute bottom-0 left-0 right-0 p-8">
                <div class="flex items-center gap-3 mb-3">
                  <span
                    ...
                    >${categoryfilter[index].category}</span
                  >
                  <span
                    class="px-3 py-1  ${categoryfilter[index].area == null ? "loading" : ""}  bg-blue-500 text-white text-sm font-semibold rounded-full"
                    >${categoryfilter[index].area}</span
                  >
                  
                </div>
                <h1 class="text-3xl md:text-4xl font-bold text-white mb-2">
                 ${categoryfilter[index].name}
                </h1>
...
`;
  let cart2 = "";
  for (let i = 0; i < categoryfilter[index].ingredients.length; i++) {
    cart2 += ` <div
                  ...
                  >
                    ...
                    <span class="text-gray-700">
                      <span class="font-medium text-gray-900">${categoryfilter[index].ingredients[i].measure} </span>
                      ${categoryfilter[index].ingredients[i].ingredient}
                    </span>
                  </div>`;
  }
  ...
  let cart3 = "";
  let num = 0;
  for (let i = 0; i < categoryfilter[index].instructions.length; i++) {
    num += 1;
    cart3 += ` <div
                  ...
                  >
                    ...
                    <p class="text-gray-700 leading-relaxed pt-2">
                       ${categoryfilter[index].instructions[i]}
                    </p>
                  </div>`;
  }

  vedio.innerHTML = `  <iframe
                    src="${categoryfilter[index].youtube.replace("watch?v=", "embed/")}"
  ...
  ingredcount.innerHTML = `${categoryfilter[index].ingredients.length} items`;
...
async function anylayze3(index) {
  try {
    const ingredients = categoryfilter[index].ingredients.map(
    ...
    const req = await fetch(
      "https://nutriplan-api.vercel.app/api/nutrition/analyze",
      {
        ...
        body: JSON.stringify({
          recipeName: categoryfilter[index].name,
...
window.log3 = function (index) {
  let details = {
    image: categoryfilter[index].thumbnail,
